...
def setup_logging():
    """
    Configura sistema de logging para debug de sincronização
    """
    # Criar diretório de logs
    os.makedirs('logs', exist_ok=True)
    
    # Configurar logger principal
    logger = logging.getLogger('SistemaAcademico')
    logger.setLevel(logging.DEBUG)
    
    # Handler para arquivo (rotação a cada 5MB, mantém 3 backups)
...
# ==============================================================================
# FUNÇÕES DE GERENCIAMENTO DE NOTAS (VERSÃO CORRIGIDA)
# ==============================================================================

def load_notas_dat(path=None):
    """
    Carrega notas do arquivo binário notas.dat
    
    IMPORTANTE: Apenas formato binário é aceito (removido fallback de texto)
    Formato: struct '<i4f' -> matricula:int32, np1:float, np2:float, pim:float, media:float
    
    Returns:
        dict: {matricula_str: (np1_str, np2_str, pim_str, media_str)}
    """
    if path is None:
    ...
    try:
        ...
        if not os.path.exists(path):
            print(f"[INFO] Arquivo notas.dat não encontrado: {path}")
            return {}
        ...
        with open(path, 'rb') as f:
            ...
            if len(data) == 0:
                print(f"[INFO] Arquivo notas.dat vazio: {path}")
                return {}
            
            if len(data) % rec_size != 0:
                print(f"[ERROR] Arquivo corrompido - tamanho inválido: {path}")
                print(f"[DEBUG] Tamanho esperado: múltiplo de {rec_size}, atual: {len(data)}")
                return {}
        ...
        print(f"[SUCCESS] Carregadas {len(notas)} notas de {path}")
        return notas
        
    except FileNotFoundError:
        print(f"[INFO] Arquivo não encontrado: {path}")
        return {}
    except struct.error as e:
        print(f"[ERROR] Erro ao desempacotar dados binários: {e}")
        return {}
    except Exception as e:
        print(f"[ERROR] Erro inesperado ao ler notas.dat: {e}")
        return {}


def save_notas_dat(notas_dict, path=None):
    """
    Salva notas atomicamente em formato binário
    
    Args:
        notas_dict: dict {matricula_str: (np1, np2, pim, media)}
    ...
    try:
        ...
        with open(tmp, 'wb') as f:
            for mat_str, notas_tuple in notas_dict.items():
                try:
                    mat = int(mat_str)
                    np1, np2, pim, media = notas_tuple
                    
                    # Converter para float se necessário
                    np1 = float(np1) if isinstance(np1, str) else np1
                    np2 = float(np2) if isinstance(np2, str) else np2
                    pim = float(pim) if isinstance(pim, str) else pim
                    media = float(media) if isinstance(media, str) else media
                    
                    f.write(struct.pack(rec_fmt, mat, np1, np2, pim, media))
                    
                except (ValueError, TypeError) as e:
                    print(f"[WARNING] Ignorando matrícula {mat_str} - dados inválidos: {e}")
                    continue
        
        # Substituir arquivo original atomicamente
        os.replace(tmp, path)
        print(f"[SUCCESS] Salvas {len(notas_dict)} notas em {path}")
        return True
        
    except Exception as e:
        print(f"[ERROR] Falha ao salvar notas.dat: {e}")
...
def update_nota_in_dat(matricula, np1, np2, pim, media, path=None):
    """
    Atualiza ou insere uma nota específica no notas.dat
    
    Args:
        matricula: matrícula do aluno (int ou str)
        np1, np2, pim, media: notas (float)
        path: caminho do arquivo (opcional)
    ...
    try:
        ...
        notas_dict = load_notas_dat(path)
        
        # Atualizar/inserir nota
        ...
        # Salvar de volta
        return save_notas_dat(notas_dict, path)
        
    except Exception as e:
        print(f"[ERROR] Falha ao atualizar nota no dat: {e}")
        return False


def delete_nota_from_dat(matricula, path=None):
    """
    Remove uma nota do notas.dat
    ...
    try:
        ...
        # Remover nota se existir
        mat_str = str(matricula)
        if mat_str in notas_dict:
            del notas_dict[mat_str]
            print(f"[INFO] Removida nota da matrícula {mat_str}")
            return save_notas_dat(notas_dict, path)
        else:
            print(f"[WARNING] Matrícula {mat_str} não encontrada em notas.dat")
            return False
            
    except Exception as e:
        print(f"[ERROR] Falha ao deletar nota do dat: {e}")
        return False
...
def get_turno_turma(id_turma):
    """Retorna o turno de uma turma específica"""
    turnos = load_turnos_turmas()
    return turnos.get(str(id_turma), 'matutino')

def set_turno_turma(id_turma, turno):
    """Define o turno de uma turma"""
    turnos = load_turnos_turmas()
    turnos[str(id_turma)] = turno
    return save_turnos_turmas(turnos)
...
def get_nota_exame(matricula):
    """Retorna a nota de exame de um aluno"""
    exames = load_exames()
    return exames.get(str(matricula), 0.0)

def set_nota_exame(matricula, nota):
    """Define a nota de exame de um aluno"""
    exames = load_exames()
    exames[str(matricula)] = float(nota)
    return save_exames(exames)
...
def get_provas_turma(id_turma):
    """Retorna as datas de provas de uma turma específica"""
    provas = load_provas()
    return provas.get(str(id_turma), {
        'NP1': None,
        'NP2': None,
        'PIM': None,
        'Exame': None
...
def set_provas_turma(id_turma, np1=None, np2=None, pim=None, exame=None):
    ...
    provas = load_provas()
    if str(id_turma) not in provas:
        provas[str(id_turma)] = {}
    
    if np1 is not None:
        provas[str(id_turma)]['NP1'] = np1
    if np2 is not None:
        provas[str(id_turma)]['NP2'] = np2
    if pim is not None:
        provas[str(id_turma)]['PIM'] = pim
    if exame is not None:
        provas[str(id_turma)]['Exame'] = exame
    
    return save_provas(provas)
...
def run_server():
    ...
    class Aluno(ctypes.Structure):
        _fields_ = [
            ...
            ("notas", Notas) 
            # Removidos os campos extras que não existem na struct C
        ]
    ...
    try:
        ...
        # Adicionada a definição para salvar_notas
        lib.salvar_notas.argtypes = [ctypes.c_int, ctypes.POINTER(Notas)]
        lib.salvar_notas.restype = ctypes.c_int

        lib.turma_existe.argtypes = [ctypes.c_int]
        lib.turma_existe.restype = ctypes.c_int

        lib.matricula_existe.argtypes = [ctypes.c_int]
        lib.matricula_existe.restype = ctypes.c_int

        lib.listar_turmas.argtypes = [ctypes.POINTER(Turma), ctypes.c_int]
        lib.listar_turmas.restype = ctypes.c_int

        lib.listar_alunos_por_turma.argtypes = [ctypes.c_int, ctypes.POINTER(Aluno), ctypes.c_int]
        lib.listar_alunos_por_turma.restype = ctypes.c_int

        lib.buscar_turma_por_id.argtypes = [ctypes.c_int, ctypes.POINTER(Turma)]
        lib.buscar_turma_por_id.restype = ctypes.c_int

        lib.buscar_aluno_por_matricula.argtypes = [ctypes.c_int, ctypes.POINTER(Aluno)]
        lib.buscar_aluno_por_matricula.restype = ctypes.c_int

        lib.atualizar_turma.argtypes = [ctypes.c_int, ctypes.c_char_p, ctypes.c_char_p]
        lib.atualizar_turma.restype = ctypes.c_int

        lib.atualizar_aluno.argtypes = [ctypes.c_int, ctypes.c_char_p]
        lib.atualizar_aluno.restype = ctypes.c_int

        lib.deletar_turma.argtypes = [ctypes.c_int]
        lib.deletar_turma.restype = ctypes.c_int

        lib.deletar_aluno.argtypes = [ctypes.c_int]
        lib.deletar_aluno.restype = ctypes.c_int
    ...
    def handle_client(conn, addr):
        ...
        try:
            while True:
                ...
                if command == "ADD_TURMA":
                    with file_lock:
                        ...
                        if not lib:
                        ...
                        elif lib.turma_existe(id_turma):
                            ...
                            lib.salvar_turma(ctypes.byref(turma))
                            response = "SUCESSO: Turma adicionada."
                
                elif command == "LIST_TURMAS":
                    if not lib:
                        response = "Nenhuma turma cadastrada."
                    else:
                        TurmasArray = Turma * 100
                        turmas = TurmasArray()
                        ...
                        if count == 0:
                            ...
                            for i in range(count):
                                response_lines.append(f"ID: {turmas[i].id}, Disciplina: {turmas[i].nome_disciplina.decode('utf-8')}, Prof: {turmas[i].nome_professor.decode('utf-8')}")
                            response = "\n".join(response_lines)
                    ...
                    with file_lock:
                        ...
                        if not lib:
                            response = "ERRO: Biblioteca C não carregada."
                        elif lib.matricula_existe(matricula):
                            response = "ERRO: Matrícula já cadastrada."
                        else:
                            # Criar notas vazias (zeros)
                            notas_vazias = Notas(np1=0.0, np2=0.0, pim=0.0, media=0.0)
                            aluno = Aluno(id_turma=id_turma, matricula=matricula, nome=parts[3].encode('utf-8'), notas=notas_vazias)
                            lib.salvar_aluno(ctypes.byref(aluno))
                            response = "SUCESSO: Aluno adicionado."
                    ...
                    if not lib:
                        ...
                        if count == 0:
                            ...
                            response_lines = []
                            # Notas agora vêm APENAS do banco C (removido fallback de notas.dat corrompido)
                            # notas_fallback = load_notas_dat()  # DESABILITADO - usa apenas banco C
                            
                            for i in range(count):
                                aluno_atual = alunos[i]
                                matricula_str = str(aluno_atual.matricula)
                                
                                # Pega notas da struct C como base
                                np1, np2, pim, media = aluno_atual.notas.np1, aluno_atual.notas.np2, aluno_atual.notas.pim, aluno_atual.notas.media
                                
                                # Banco C é a única fonte de verdade (fallback removido)

                                # Pega a nota de exame do arquivo separado
                                exame = get_nota_exame(matricula_str)
                                
                                response_lines.append(
                                    f"Matrícula: {aluno_atual.matricula}, Nome: {aluno_atual.nome.decode('utf-8')}, "
                    ...
                    with file_lock:
                        if lib and lib.deletar_aluno(int(parts[1])):
                    ...
                    """
                    Atualiza notas de um aluno no banco de dados C.
                    
                    CORREÇÃO: Removido o fallback para notas.dat para garantir consistência.
                    O banco de dados C é agora a única fonte de verdade para salvar e ler notas.
                    """
                    try:
                        ...
                        logger.info(f"[SERVER] UPDATE_NOTAS - Matrícula: {matricula}, NP1: {np1}, NP2: {np2}, PIM: {pim}, Média: {media}")
                        
                        success = False
                        
                        # Tenta salvar no banco C como única fonte de verdade
                        if lib:
                            notas = Notas(np1=np1, np2=np2, pim=pim, media=media)
                            # A função salvar_notas retorna 1 em sucesso, 0 em falha
                            if lib.salvar_notas(matricula, ctypes.byref(notas)):
                                success = True
                                logger.info(f"[SERVER] ✓ Notas salvas no banco C para matrícula {matricula}")
                            else:
                                logger.error(f"[SERVER] ✗ Falha ao salvar no banco C para matrícula {matricula} (matrícula não encontrada no DB C?)")
                        else:
                            logger.error("[SERVER] ✗ Biblioteca C não carregada, não foi possível salvar notas.")
                        
                        # Responde ao cliente com base no sucesso da operação no banco C
                        if success:
                            response = "SUCESSO: Notas atualizadas no banco C."
                        else:
                            response = "ERRO: Falha ao salvar notas no banco de dados principal. Verifique se a matrícula existe."
                            
                    except (IndexError, ValueError) as e:
                        logger.error(f"[SERVER] ✗ Formato inválido para UPDATE_NOTAS: {e}")
                        response = f"ERRO: Formato de dados inválido - {str(e)}"
                    except Exception as e:
                        logger.error(f"[SERVER] ✗ Erro inesperado em UPDATE_NOTAS: {e}", exc_info=True)
                        response = f"ERRO: Ocorreu um erro inesperado no servidor - {str(e)}"
                    ...
                    if not os.path.exists(turma_folder):
                        ...
                        response = "\n".join(files) if files else "Nenhuma atividade encontrada para esta turma."
            ...
            print(f"[SERVIDOR-AVISO] Conexão com {addr} perdida: {e}")
        except Exception as e:
...
# Configuração dinâmica de HOST baseada na disponibilidade do servidor
SERVER_HOST = '10.101.108.58'
FALLBACK_HOST = '0.0.0.0'
PORT = 65432

def test_server_connection(host, port, timeout=3):
    """Testa se o servidor está disponível no host especificado"""
    try:
...
def get_available_host():
    """Retorna o host disponível: servidor principal ou fallback local"""
    if test_server_connection(SERVER_HOST, PORT):
        print(f"[CONEXÃO] Servidor principal disponível: {SERVER_HOST}")
        return SERVER_HOST
    else:
        print(f"[CONEXÃO] Usando dados locais (servidor não disponível)")
        return FALLBACK_HOST

# HOST será determinado dinamicamente
HOST = get_available_host()

# Funções para gerenciamento de dados locais quando servidor não disponível
def is_using_local_data():
    """Verifica se está usando dados locais (fallback)"""
    return HOST == FALLBACK_HOST

def load_local_turmas():
    """Carrega turmas do arquivo local"""
    try:
        if os.path.exists('turmas_local.json'):
            with open('turmas_local.json', 'r', encoding='utf-8') as f:
                return json.load(f)
        return {}
    except:
        return {}

def save_local_turmas(turmas_data):
    """Salva turmas no arquivo local"""
    try:
        with open('turmas_local.json', 'w', encoding='utf-8') as f:
            json.dump(turmas_data, f, indent=2, ensure_ascii=False)
        return True
    except:
        return False

def load_local_alunos():
    """Carrega alunos do arquivo local"""
    try:
        if os.path.exists('alunos_local.json'):
            with open('alunos_local.json', 'r', encoding='utf-8') as f:
                return json.load(f)
        return {}
    except:
        return {}

def save_local_alunos(alunos_data):
    """Salva alunos no arquivo local"""
    try:
        with open('alunos_local.json', 'w', encoding='utf-8') as f:
            json.dump(alunos_data, f, indent=2, ensure_ascii=False)
        return True
    except:
        return False

def load_local_notas():
    """Carrega notas do arquivo local"""
    try:
        if os.path.exists('notas_local.json'):
            with open('notas_local.json', 'r', encoding='utf-8') as f:
                return json.load(f)
        return {}
    except:
        return {}

def save_local_notas(notas_data):
    """Salva notas no arquivo local"""
    try:
        with open('notas_local.json', 'w', encoding='utf-8') as f:
            json.dump(notas_data, f, indent=2, ensure_ascii=False)
        return True
    except:
        return False
...
class App(tk.Tk):
    ...
    def _send_request(self, request, buffer=8192):
        """Envia requisição para o servidor ou usa dados locais como fallback"""
        # Se estiver usando dados locais, processa localmente
        if is_using_local_data():
            return self._process_local_request(request)
        ...
        try:
            ...
            # Se falhar, tenta usar dados locais como fallback
            print(f"[FALLBACK] Erro de conexão: {e}. Usando dados locais.")
            return self._process_local_request(request)

    def _process_local_request(self, request):
        """Processa requisições localmente quando o servidor não está disponível"""
        try:
            parts = request.split('|')
            command = parts[0]
            
            if command == "LIST_TURMAS":
                return self._list_local_turmas()
            elif command == "LIST_ALUNOS_POR_TURMA":
                turma_id = parts[1] if len(parts) > 1 else ""
                return self._list_local_alunos_por_turma(turma_id)
            elif command == "GET_TURMA_DATA":
                turma_id = parts[1] if len(parts) > 1 else ""
                return self._get_local_turma_data(turma_id)
            elif command == "ADD_TURMA":
                if len(parts) >= 4:
                    turma_id, disciplina, professor = parts[1], parts[2], parts[3]
                    return self._add_local_turma(turma_id, disciplina, professor)
            elif command == "ADD_ALUNO":
                if len(parts) >= 4:
                    turma_id, matricula, nome = parts[1], parts[2], parts[3]
                    return self._add_local_aluno(turma_id, matricula, nome)
            elif command == "UPDATE_NOTAS":
                if len(parts) >= 6:
                    matricula, np1, np2, pim, media = parts[1], parts[2], parts[3], parts[4], parts[5]
                    return self._update_local_notas(matricula, np1, np2, pim, media)
            else:
                return "Comando não suportado em modo local"
                
        except Exception as e:
            print(f"[LOCAL] Erro ao processar requisição: {e}")
            return "Erro ao processar requisição local"

    def _list_local_turmas(self):
        """Lista turmas do arquivo local"""
        turmas = load_local_turmas()
        if not turmas:
            return "Nenhuma turma cadastrada."
        
        result = []
        for turma_id, turma_data in turmas.items():
            result.append(f"ID: {turma_id}, Disciplina: {turma_data.get('disciplina', '')}, Professor: {turma_data.get('professor', '')}")
        return '\n'.join(result)

    def _list_local_alunos_por_turma(self, turma_id):
        """
        Lista alunos de uma turma específica do arquivo local.
        
        CORREÇÃO: Agora, esta função também carrega as notas de 'notas_local.json'
        e constrói a resposta no mesmo formato do servidor principal para consistência.
        """
        alunos = load_local_alunos()
        notas = load_local_notas()  # Carrega as notas salvas localmente
        
        turma_alunos = alunos.get(str(turma_id), {})
        if not turma_alunos:
            return "Nenhum aluno cadastrado nesta turma."
        
        result = []
        for matricula, aluno_data in turma_alunos.items():
            nome = aluno_data.get('nome', '')
            
            # Busca as notas do aluno no arquivo de notas local
            notas_aluno = notas.get(str(matricula), {})
            np1 = notas_aluno.get('np1', 0.0)
            np2 = notas_aluno.get('np2', 0.0)
            pim = notas_aluno.get('pim', 0.0)
            media = notas_aluno.get('media', 0.0)
            
            # Busca a nota de exame do arquivo separado (mantém consistência)
            exame = get_nota_exame(matricula)
            
            # Constrói a linha de resposta no formato EXATO do servidor
            linha_formatada = (
                f"Matrícula: {matricula}, Nome: {nome}, "
                f"NP1: {np1:.1f}, NP2: {np2:.1f}, PIM: {pim:.1f}, "
                f"Média: {media:.1f}, Exame: {exame:.1f}"
            )
            result.append(linha_formatada)
            
        return '\n'.join(result)

    def _get_local_turma_data(self, turma_id):
        """Obtém dados de uma turma específica do arquivo local"""
        turmas = load_local_turmas()
        turma_data = turmas.get(turma_id)
        if not turma_data:
            return "ERRO: Turma não encontrada"
        
        disciplina = turma_data.get('disciplina', '')
        professor = turma_data.get('professor', '')
        return f"{disciplina}|{professor}"

    def _add_local_turma(self, turma_id, disciplina, professor):
        """Adiciona uma nova turma ao arquivo local"""
        turmas = load_local_turmas()
        turmas[turma_id] = {
        ...
        if save_local_turmas(turmas):
            return "Turma adicionada com sucesso"
        else:
            return "ERRO: Falha ao salvar turma"

    def _add_local_aluno(self, turma_id, matricula, nome):
        """Adiciona um novo aluno ao arquivo local"""
        alunos = load_local_alunos()
        if turma_id not in alunos:
            alunos[turma_id] = {}
        
        alunos[turma_id][matricula] = {
            'nome': nome,
            'created_at': str(datetime.now())
        }
        
        if save_local_alunos(alunos):
            return "SUCESSO: Aluno adicionado"
        else:
            return "ERRO: Falha ao salvar aluno"

    def _update_local_notas(self, matricula, np1, np2, pim, media):
        """Atualiza notas de um aluno no arquivo local"""
        notas = load_local_notas()
        
        # Criar estrutura de notas por matrícula (sem turma_id)
        notas[matricula] = {
            'np1': float(np1),
            'np2': float(np2),
            'pim': float(pim),
        ...
        if save_local_notas(notas):
            return "SUCESSO: Notas atualizadas com sucesso"
        else:
            return "ERRO: Falha ao salvar notas"

### CORREÇÃO: Busca professores a partir dos dados do servidor ###
    def _get_professors_from_server(self):
        """Busca a lista de professores e suas turmas a partir dos dados do servidor."""
        # Se estiver em modo local, usa a base de dados JSON como fallback
        if is_using_local_data():
            return self.db.get_all_professors()

        turmas_resp = self._send_request("LIST_TURMAS")
        if not turmas_resp or "Nenhuma turma" in turmas_resp:
            return []

        professores = {}  # Usar um dicionário para agrupar turmas por professor
        for linha in turmas_resp.strip().split('\n'):
            try:
                partes = {p.split(': ')[0].strip(): p.split(': ')[1].strip() for p in linha.split(', ')}
                prof_nome = partes.get('Prof')
                turma_id = partes.get('ID')
                
                if prof_nome and turma_id:
                    # Gera um username consistente a partir do nome
                    username = self.db.generate_username_from_name(prof_nome)
                    
                    if username not in professores:
                        # Se for a primeira vez que vemos este professor, cria a entrada
                        professores[username] = {
                            'username': username,
                            'email': self.db.get_user_data(username).get('email', 'Não informado') if self.db.get_user_data(username) else 'Não informado',
                            'subjects': []
                        }
                    # Adiciona a turma à lista de matérias do professor
                    professores[username]['subjects'].append(turma_id)
            except (IndexError, KeyError):
                continue
        
        return list(professores.values())

    ### CORREÇÃO: Busca todos os usuários (professores e alunos) do servidor ###
    def _get_all_users_from_server(self):
        """Busca uma lista consolidada de todos os usuários (alunos e professores) do servidor."""
        if is_using_local_data():
        ...
        if turmas_resp and "Nenhuma turma" not in turmas_resp:
            for linha in turmas_resp.strip().split('\n'):
                try:
                    ...
                    if prof_nome:
                        prof_username = self.db.generate_username_from_name(prof_nome)
                        if prof_username not in all_users:
                            all_users[prof_username] = {'username': prof_username, 'role': 'professor'}

                    # Busca e adiciona os alunos da turma
                    if turma_id:
                        alunos_resp = self._send_request(f"LIST_ALUNOS_POR_TURMA|{turma_id}")
                        if alunos_resp and "Nenhum aluno" not in alunos_resp:
                            for aluno_linha in alunos_resp.strip().split('\n'):
                                try:
                                    aluno_partes = {p.split(': ')[0].strip(): p.split(': ')[1].strip() for p in aluno_linha.split(', ')}
                                    matricula = aluno_partes.get('Matrícula')
                                    nome_aluno = aluno_partes.get('Nome')
                                    # Usa matrícula como username para consistência
                                    if matricula not in all_users:
                                        all_users[matricula] = {
                                            'username': nome_aluno, 
                                            'role': 'aluno', 
                                            'matricula': matricula
        ...
        # Complementa com dados do users.json (como email, status, etc.)
        for username, user_data in all_users.items():
            local_data = self.db.get_user_data(username)
            if local_data:
                user_data.update(local_data)
    ...
    def cadastrar_turma(self):
        ...
        def cadastrar():
            ...
            # Cria um usuário para o professor se ele não existir
            ...
            if not user_exists:
                self.db.add_user(prof_username, prof_password, 'professor', turno=turno_var.get(), pending=False)

            # Envia para o servidor
            resp = self._send_request(f"ADD_TURMA|{novo_id}|{disciplina}|{professor}")
            
            if resp and "SUCESSO" in resp:
                # Associa a turma ao professor no banco de usuários
                subjects = self.db.get_professor_subjects(prof_username)
                if str(novo_id) not in subjects:
                    subjects.append(str(novo_id))
                    self.db.set_professor_subjects(prof_username, subjects)
                
                # Salva o turno
                set_turno_turma(novo_id, turno_var.get())

                msg_sucesso = f"Turma '{disciplina}' cadastrada com ID: {novo_id}\n\n"
                if not user_exists:
                    msg_sucesso += f"Login criado para o professor:\nUsuário: {prof_username}\nSenha: {prof_password}"
                
                messagebox.showinfo("Sucesso", msg_sucesso)
                dialog.destroy()
                self.listar_turmas() # Atualiza a lista principal
            else:
                messagebox.showerror("Erro", resp or "Falha ao cadastrar turma.")
        ...
        btn_ok = tk.Button(button_frame, text="Cadastrar", 
                          command=cadastrar,
                          bg=self.colors['primary'], fg='white',
                          font=('Arial', 10, 'bold'), relief='flat', cursor='hand2',
                          padx=20, pady=8)
        btn_ok.pack(side=tk.RIGHT, padx=(0, 5))
    ...
    def _get_alunos_list(self, id_turma=None):
        ...
        alunos = []
        if id_turma:
            print(f"[DEBUG] Buscando alunos para turma {id_turma}")
            ...
            if not resp or "Nenhum aluno" in resp:
                print(f"[DEBUG] Nenhum aluno encontrado para turma {id_turma}")
                return alunos
            for linha in resp.strip().split('\n'):
                if not linha: continue
                partes = linha.split(', ')
    ...
    def _refresh_pesquisa_aluno(self, dialog, search_frame, aluno_var, alunos_disponiveis):
        """Atualiza a lista de alunos na pesquisa"""
        # Limpar combobox atual
        for widget in search_frame.winfo_children():
            if isinstance(widget, ttk.Combobox):
                widget.destroy()
        
        # Recarregar lista de alunos
        alunos_disponiveis.clear()
        ...
        messagebox.showinfo("Atualização", f"Lista atualizada! {len(alunos_disponiveis)} aluno(s) encontrado(s).")
    ...
    def pesquisar_aluno(self):
        ...
        # Botão atualizar
        refresh_btn = ttk.Button(button_frame, text="🔄 Atualizar Lista", 
                                command=lambda: self._refresh_pesquisa_aluno(dialog, search_frame, aluno_var, alunos_disponiveis),
                                style='Secondary.TButton')
        refresh_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        # Botão fechar
        close_btn = ttk.Button(button_frame, text="Fechar", command=dialog.destroy,
                  style='Secondary.TButton')
    ...
    def _refresh_pesquisa_professor(self, dialog, search_frame, prof_var, professores_disponiveis):
        """Atualiza a lista de professores na pesquisa"""
        # Limpar combobox atual
        for widget in search_frame.winfo_children():
            if isinstance(widget, ttk.Combobox):
                widget.destroy()
        
        # Recarregar lista de professores diretamente das turmas do servidor (igual à pesquisa de alunos)
        professores_disponiveis.clear()
            ...
            for linha in turmas_resp.strip().split('\n'):
                if linha:
                    ...
                    if len(partes) >= 3:
                        ...
                        professor_nome = partes[2].split(': ')[1]
                        
                        # Evitar duplicados
                        if not any(p['nome'] == professor_nome for p in professores_disponiveis):
        ...
        prof_values = [p['nome'] for p in professores_disponiveis]
        ...
        messagebox.showinfo("Atualização", f"Lista atualizada! {len(professores_disponiveis)} professor(es) encontrado(s).")
    ...
    def pesquisar_professor(self):
        ...
        # Coletar todos os professores diretamente das turmas do servidor (igual à pesquisa de alunos)
        ...
        prof_values = [f"{p['nome']}" for p in professores_disponiveis]
        prof_combo = ttk.Combobox(search_frame, textvariable=prof_var,
                                   values=prof_values,
                                   state="readonly", width=60, font=('Arial', 10))
        prof_combo.pack(fill=tk.X, pady=(0, 10))
        ...
        def exibir_informacoes():
            ...
            professor_nome = prof_var.get()
            
            # Buscar informações do professor
            prof_info = next((p for p in professores_disponiveis if p['nome'] == professor_nome), None)
            if not prof_info:
                return
            
            # Buscar dados do usuário usando o username gerado
            prof_username = self.db.generate_username_from_name(professor_nome)
            user_data = self.db.users.get(prof_username, {})
            ...
            # Buscar turmas associadas diretamente do servidor (igual à pesquisa de alunos)
            turmas_associadas = []
            turmas_resp = self._send_request("LIST_TURMAS")
            if turmas_resp and "Nenhuma turma" not in turmas_resp:
                for linha in turmas_resp.strip().split('\n'):
                    if linha:
                        ...
                        if len(partes) >= 3:
                            tid = partes[0].split(': ')[1]
                            disciplina = partes[1].split(': ')[1]
                            professor_nome_turma = partes[2].split(': ')[1]
                            
                            # Se o professor desta turma é o professor selecionado
                            if professor_nome_turma == professor_nome:
        ...
        # Botão atualizar
        refresh_btn = ttk.Button(button_frame, text="🔄 Atualizar Lista", 
                                command=lambda: self._refresh_pesquisa_professor(dialog, search_frame, prof_var, professores_disponiveis),
                                style='Secondary.TButton')
        refresh_btn.pack(side=tk.LEFT, padx=(0, 10))
    ...
    def _combined_turma_aluno_selector(self, title="Selecionar Turma/Aluno"):
        ...
        print(f"[DEBUG] Total de turmas encontradas: {len(turmas)}")
        for t in turmas:
            print(f"[DEBUG] Turma: {t['id']} - {t['nome_disciplina']}")
        ...
        if self.role == 'professor':
            ...
            turmas_acessiveis = []
            for t in turmas:
                has_access = self.db.can_access_subject(self.username, t['id'])
                print(f"[DEBUG] Professor {self.username} - Turma {t['id']} - Acesso: {has_access}")
                if has_access:
                    turmas_acessiveis.append(t)
            turmas = turmas_acessiveis
            print(f"[DEBUG] Turmas acessíveis para professor: {len(turmas)}")
        ...
        def refresh_alunos(event=None):
            ...
            if not sel_tid:
                aluno_combo['values'] = []
                return
            print(f"[DEBUG] Atualizando lista de alunos para turma {sel_tid}")
            alunos = self._get_alunos_list(sel_tid)
            print(f"[DEBUG] Alunos encontrados: {len(alunos)}")
            aluno_combo['values'] = [a[1] for a in alunos]
            if alunos:
                aluno_combo.current(0)
            else:
                aluno_combo.set('')
                print(f"[DEBUG] Nenhum aluno disponível para seleção")
...
### MELHORIA: Gera o ID da turma dinamicamente ###
    def _gerar_proximo_id_turma(self):
        resp = self._send_request("LIST_TURMAS")
        if not resp or "Nenhuma turma" in resp:
            return 1
        
        ids = [int(linha.split(', ')[0].split(': ')[1]) for linha in resp.strip().split('\n') if linha]
        return max(ids) + 1 if ids else 1

    ### MELHORIA: Gera a matrícula do aluno dinamicamente ###
    def _gerar_proxima_matricula(self):
        ...
        if turmas_resp and "Nenhuma turma" not in turmas_resp:
            for linha in turmas_resp.strip().split('\n'):
                try:
                    id_turma = linha.split(', ')[0].split(': ')[1]
                    alunos_resp = self._send_request(f"LIST_ALUNOS_POR_TURMA|{id_turma}")
                    if alunos_resp and "Nenhum aluno" not in alunos_resp:
                        for aluno_linha in alunos_resp.strip().split('\n'):
                            matricula = int(aluno_linha.split(', ')[0].split(': ')[1])
                            if matricula > max_mat:
                                max_mat = matricula
                except (IndexError, ValueError):
                    continue
        
        # Define um valor inicial se não houver matrículas
        return max_mat + 1 if max_mat > 0 else 20250001
    ...
    def cadastrar_aluno(self):
        ...
        def cadastrar():
            ...
            if not user_exists:
                success, msg = self.db.add_user(aluno_username, aluno_password, 'aluno', turno=turno_turma, pending=False, matricula=nova_matricula)
                if not success:
                    messagebox.showerror("Erro ao criar usuário", msg)
                    return
            else:
                success, msg = self.db.set_student_matricula(aluno_username, nova_matricula)
                if not success:
                    messagebox.showerror("Erro ao associar matrícula", msg)
                    return

            # Envia para o servidor
            try:
                request_str = f"ADD_ALUNO|{tid}|{nova_matricula}|{nome_aluno}"
                logger.info(f"Enviando requisição: {request_str}")
                resp = self._send_request(request_str)
                logger.info(f"Resposta recebida: {resp}")
                
                if resp and "SUCESSO" in resp:
                    msg_sucesso = f"Aluno '{nome_aluno}' cadastrado com matrícula: {nova_matricula}\n\n"
                    if not user_exists:
                        msg_sucesso += f"Login criado para o aluno:\nUsuário: {aluno_username}\nSenha: {aluno_password}"
                    
                    messagebox.showinfo("Sucesso", msg_sucesso)
                    dialog.destroy()
                else:
                    messagebox.showerror("Erro ao Cadastrar", f"Resposta do servidor:\n{resp or 'Sem resposta do servidor'}")
            except Exception as e:
                logger.error(f"Erro ao cadastrar aluno: {str(e)}", exc_info=True)
    ...
    def _refresh_aluno_list(self, id_t):
        ...
        CORREÇÃO: Unificada a lógica de cálculo de status com a da função 'pesquisar_aluno'
        e corrigida a estrutura de dados para exibição correta na tabela.
        """
        if not id_t:
            return
        ...
        if self.role != 'admin' and turno_turma != turno_usuario:
            messagebox.showerror("Acesso Negado", 
        ...
        # 1. Coleta os dados brutos do servidor
        alunos_data_bruta = []
        for linha in resp.strip().split('\n'):
            if not linha: 
                continue
            ...
            for p in partes[2:]:
                try:
                    if p.startswith('NP1:'): np1 = float(p.split(': ')[1])
                    elif p.startswith('NP2:'): np2 = float(p.split(': ')[1])
                    elif p.startswith('PIM:'): pim = float(p.split(': ')[1])
                    elif p.startswith('Média:'): media = float(p.split(': ')[1])
                    elif p.startswith('Exame:'): exame = float(p.split(': ')[1])
                except (ValueError, IndexError):
                    continue # Ignora partes mal formatadas
            
            alunos_data_bruta.append([matricula, nome, np1, np2, pim, media, exame])
        ...
        try:
            ...
            print(f"[WARNING] Erro ao carregar presenças: {e}")

        # 3. Processa os dados brutos para criar a lista final de exibição
        final_display_data = []
        for row in alunos_data_bruta:
            matricula, nome, np1_val, np2_val, pim_val, media_val, exame_val = row
            
            faltas = faltas_map.get(str(matricula), 0)

            # --- LÓGICA DE STATUS CORRIGIDA (idêntica a 'pesquisar_aluno') ---
            tem_notas = (np1_val > 0 or np2_val > 0 or pim_val > 0)
            if not tem_notas:
                status = "Pendente Atribuir Notas"
            elif media_val >= 7.0:
                status = "Aprovado"
            # O exame só é relevante se a média for < 7
            elif media_val < 7.0:
                if exame_val == 0.0:
                    status = "Pendente Exame"
                elif exame_val >= 5.0:
                    status = "Aprovado (Exame)"
                else:  # Cobre o caso de exame > 0 mas < 5
                    status = "Reprovado"
            else:
                status = "Pendente" # Fallback, não deve ser atingido

            turno_icone = {'matutino': '☀️', 'vespertino': '🌤️', 'noturno': '🌙'}.get(turno_turma, '🕐')
            turno_display = f"{turno_icone} {turno_turma.title()}"

            # 4. Constrói a linha final na ordem correta dos headers
            final_row = [
                matricula, 
                nome, 
                f"{np1_val:.1f}", 
            ...
            final_display_data.append(final_row)

        headers = ["Matrícula", "Nome", "NP1", "NP2", "PIM", "Média", "Faltas", "Turno", "Exame", "Status"]
        self._update_display(f"Alunos da Turma {id_t} - {disc}", headers, final_display_data)
    ...
    def _editar_aluno_com_turma(self):
        ...
        for uname, udata in self.db.users.items():
            # A matrícula no banco de dados pode ser int, então comparamos como string para segurança
            if udata.get('role') == 'aluno' and str(udata.get('matricula')) == str(mat):
        ...
        if resp and "ERRO" not in resp:
            ...
            def trocar_turma():
                ...
                if add_resp and ("SUCESSO" in add_resp or "adicionado" in add_resp.lower()):
                    messagebox.showinfo("Sucesso", f"Aluno transferido para a turma {nova_turma_id} com sucesso!")
                    dialog.destroy()
                    self.listar_turmas()
                else:
                    messagebox.showerror("Erro", f"Erro ao adicionar aluno na nova turma: {add_resp}")
            
            ttk.Button(turma_frame, text="Trocar Turma", command=trocar_turma, 
                      style='Dialog.TButton').pack(anchor='w', padx=10, pady=(10, 0))
            
            # Aba de dados do usuário (somente se o usuário existir no banco de dados JSON)
            if username_aluno:
                ...
                # Garante que o campo de email seja preenchido com o valor atual do usuário.
            ...
            def save_changes():
                ...
                if up_resp and "SUCESSO" in up_resp:
                    if username_aluno:
                        # 'email_entry' só existe se username_aluno for verdadeiro
                        new_user_data = {'email': email_entry.get()}
    ...
    def lancar_notas(self):
        ...
        class NotasDialog:
            ...
            def atualizar_notas(self):
                """
                Atualiza notas de um aluno selecionado
                
                CORREÇÃO: Sincroniza banco C e notas.dat
                """
                selection = self.tree.selection()
                if not selection:
                ...
                try:
                    ...
                    if self.tipo_nota == "comum":
                        ...
                        print(f"[CLIENT] Enviando notas: Matrícula={matricula}, NP1={np1}, NP2={np2}, PIM={pim}, Média={media}")
                        
                        # ============================================================
                        # CORREÇÃO CRÍTICA: Enviar para servidor C
                        # ============================================================
                        resp = self.parent._send_request(f"UPDATE_NOTAS|{matricula}|{np1}|{np2}|{pim}|{round(media, 1)}")
                        
                        print(f"[CLIENT] Resposta do servidor: {resp}")
                        
                        # Verificar se houve sucesso
                        if resp and "SUCESSO" in resp:
                            # ============================================================
                            # ADICIONAL: Sincronizar notas.dat localmente (cache)
                            # ============================================================
                            try:
                                notas_path = os.path.join('uploads', 'notas.dat')
                                
                                # Atualizar notas.dat como cache
                                notas_dict = load_notas_dat(notas_path)
                                notas_dict[str(matricula)] = (f"{np1:.1f}", f"{np2:.1f}", f"{pim:.1f}", f"{media:.1f}")
                                save_notas_dat(notas_dict, notas_path)
                                
                                print(f"[CLIENT] ✓ Cache notas.dat atualizado para matrícula {matricula}")
                                
                            except Exception as e:
                                print(f"[CLIENT] ✗ WARNING: Falha ao atualizar cache notas.dat: {e}")
                                # Não é erro crítico - o banco C já foi atualizado
                            
                            # Mostrar mensagem de sucesso
                            messagebox.showinfo("Sucesso", "Notas atualizadas com sucesso!")
                            
                            # Fechar o diálogo de notas
                            self.dialog.destroy()
                            
                            # Atualizar a lista principal de alunos
                            self.parent._refresh_aluno_list(self.id_turma)
                        else:
                            messagebox.showerror("Erro", f"Falha ao atualizar notas: {resp}")
                    
                    else:  # Tipo "exame"
                        # Buscar notas atuais do servidor C
                        ...
                        # Salvar nota de exame (JSON local - correto)
                    ...
                    print(f"[ERROR] Erro ao atualizar notas: {e}")
...
• Use os botões de ação para salvar as alterações ou fechar"""
    ...
    def upload_atividade(self):
        ...
        # Se estiver usando dados locais, fazer upload local
        if is_using_local_data():
            self._upload_atividade_local(id_t, f_path)
            return
        ...
        try:
            ...
            # Se falhar, tenta upload local como fallback
            print(f"[FALLBACK] Erro de upload: {e}. Tentando upload local.")
            self._upload_atividade_local(id_t, f_path)

    def _upload_atividade_local(self, turma_id, file_path):
        """Faz upload de arquivo para armazenamento local"""
        try:
            # Criar pasta local para a turma
            local_folder = f"atividades_local/turma_{turma_id}"
            os.makedirs(local_folder, exist_ok=True)
            
            # Copiar arquivo para pasta local
            file_name = os.path.basename(file_path)
            timestamp = int(time.time())
            local_file_path = os.path.join(local_folder, f"{timestamp}_{file_name}")
            
            with open(file_path, 'rb') as src, open(local_file_path, 'wb') as dst:
                dst.write(src.read())
            
            messagebox.showinfo("Upload Local", f"Arquivo '{file_name}' enviado com sucesso para armazenamento local.")
            
        except Exception as e:
            messagebox.showerror("Erro Upload Local", f"Ocorreu um erro no upload local: {e}")
    ...
    def listar_atividades(self):
        ...
        # Se estiver usando dados locais, listar arquivos locais
        if is_using_local_data():
            resp = self._list_local_files(id_t)
        else:
                ...
                for arquivo in resp.strip().split('\n'):
                    if arquivo:
                        ...
                        try:
                            ...
                            if is_using_local_data():
                                arquivo_path = os.path.join('atividades_local', f'turma_{id_t}', arquivo)
                            else:
    ...
    def _download_arquivo(self, event, id_turma):
        ...
        if save_path:
            # Se estiver usando dados locais, fazer download local
            if is_using_local_data():
                self._download_arquivo_local(id_turma, arquivo_original, save_path)
                return
            ...
            try:
                ...
                # Se falhar, tenta download local como fallback
                print(f"[FALLBACK] Erro de download: {e}. Tentando download local.")
                self._download_arquivo_local(id_turma, arquivo_original, save_path)

    def _download_arquivo_local(self, turma_id, arquivo_original, save_path):
        """Faz download de arquivo do armazenamento local"""
        try:
            # Procurar arquivo na pasta local
            local_folder = f"atividades_local/turma_{turma_id}"
            if not os.path.exists(local_folder):
                messagebox.showerror("Erro", "Pasta de atividades local não encontrada.")
                return
            
            # Procurar arquivo que contenha o nome original
            arquivos_encontrados = []
            for file_name in os.listdir(local_folder):
                if arquivo_original in file_name:
                    arquivos_encontrados.append(file_name)
            
            if not arquivos_encontrados:
                messagebox.showerror("Erro", f"Arquivo '{arquivo_original}' não encontrado no armazenamento local.")
                return
            
            # Se houver múltiplos arquivos, usar o mais recente
            arquivo_local = max(arquivos_encontrados, key=lambda x: os.path.getctime(os.path.join(local_folder, x)))
            arquivo_local_path = os.path.join(local_folder, arquivo_local)
            
            # Copiar arquivo para destino
            with open(arquivo_local_path, 'rb') as src, open(save_path, 'wb') as dst:
                dst.write(src.read())
            
            messagebox.showinfo("Download Local", f"Arquivo baixado com sucesso do armazenamento local!")
            
        except Exception as e:
            messagebox.showerror("Erro Download Local", f"Ocorreu um erro no download local: {e}")

    def _list_local_files(self, turma_id):
        """Lista arquivos da turma no armazenamento local"""
        try:
            local_folder = f"atividades_local/turma_{turma_id}"
            if not os.path.exists(local_folder):
                return "Nenhuma atividade encontrada para esta turma."
            ...
            print(f"[LOCAL] Erro ao listar arquivos: {e}")
            return "Erro ao listar arquivos locais"
    ...
    def gerenciar_acessos_professores(self):
        ...
        def load_professors():
            ...
            professors = self._get_professors_from_server()
    ...
    def listar_professores(self):
        ...
        # Usar dados do servidor quando conectado, senão usar dados locais
        professors = self._get_professors_from_server()
    ...
    def listar_usuarios_consolidado(self):
        ...
        def refresh_list():
            ...
            # Buscar usuários do servidor quando conectado
            all_users = self._get_all_users_from_server()
            usuarios = []
...
def test_notas_sync():
    """
    Testa sincronização entre banco C e notas.dat
    Execute este teste após iniciar o servidor
    """
    print("\n" + "="*60)
    print("TESTE DE SINCRONIZAÇÃO DE NOTAS")
    print("="*60)
    
    # Teste 1: Salvar nota via servidor
    print("\n[TESTE 1] Salvando nota via servidor...")
    test_matricula = 99999
    test_notas = (7.5, 8.0, 9.0, 8.0)
    
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            request = f"UPDATE_NOTAS|{test_matricula}|{test_notas[0]}|{test_notas[1]}|{test_notas[2]}|{test_notas[3]}"
            s.sendall(request.encode('utf-8'))
            ...
            if "SUCESSO" in resp:
                ...
                print("   ✗ Falha ao salvar nota")
                return False
    except Exception as e:
        print(f"   ✗ Erro de conexão: {e}")
        return False
    
    # Teste 2: Verificar se nota foi salva no notas.dat
    print("\n[TESTE 2] Verificando notas.dat...")
    notas_dat_path = os.path.join('uploads', 'notas.dat')
    
    if os.path.exists(notas_dat_path):
        notas_dict = load_notas_dat(notas_dat_path)
        if str(test_matricula) in notas_dict:
            saved_notas = notas_dict[str(test_matricula)]
            print(f"   ✓ Nota encontrada em notas.dat: {saved_notas}")
        else:
            print(f"   ⚠ Nota não encontrada em notas.dat (esperado se banco C funcionou)")
    else:
        print(f"   ⚠ Arquivo notas.dat não existe (esperado se banco C funcionou)")
    
    # Teste 3: Buscar nota do servidor
    print("\n[TESTE 3] Buscando aluno do servidor...")
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            # Assumindo que o aluno está na turma 1 (ajuste conforme necessário)
            request = f"LIST_ALUNOS_POR_TURMA|1"
            s.sendall(request.encode('utf-8'))
            resp = s.recv(4096).decode('utf-8')
            
            if str(test_matricula) in resp:
                print(f"   ✓ Aluno encontrado na listagem")
                # Extrair notas da resposta
                for linha in resp.split('\n'):
                    if str(test_matricula) in linha:
                        print(f"   Dados: {linha}")
                        break
            else:
                print(f"   ⚠ Aluno não encontrado (pode estar em outra turma)")
...
def debug_notas_file():
    """
    Exibe conteúdo do arquivo notas.dat para debug
    """
    print("\n" + "="*60)
    print("DEBUG: CONTEÚDO DE notas.dat")
    print("="*60)
    
    notas_path = os.path.join('uploads', 'notas.dat')
    
    if not os.path.exists(notas_path):
        print("   Arquivo notas.dat não existe")
        return
    ...
    try:
        ...
        print(f"   Total de registros: {len(notas_dict)}")
        print("\n   Matrícula | NP1  | NP2  | PIM  | Média")
        print("   " + "-"*50)
        
        for matricula, (np1, np2, pim, media) in sorted(notas_dict.items()):
            print(f"   {matricula:8} | {np1:4} | {np2:4} | {pim:4} | {media:5}")
...
# ==============================================================================
# FUNÇÃO DE MIGRAÇÃO DE DADOS (USE UMA VEZ APÓS A ATUALIZAÇÃO)
# ==============================================================================

def migrate_notas_to_server():
    """
    Migra todas as notas do notas.dat para o banco C
    
    IMPORTANTE: Execute esta função UMA VEZ após atualizar o código
    para sincronizar notas antigas do notas.dat com o banco C
    """
    print("\n" + "="*60)
    print("MIGRAÇÃO DE NOTAS: notas.dat → Banco C")
    print("="*60)
    
    notas_path = os.path.join('uploads', 'notas.dat')
    
    if not os.path.exists(notas_path):
        print("   ⚠ Arquivo notas.dat não encontrado. Nada a migrar.")
        return
    ...
    try:
        ...
        if not notas_dict:
            print("   ⚠ Arquivo notas.dat vazio. Nada a migrar.")
            return
        
        print(f"\n   Encontradas {len(notas_dict)} notas para migrar...")
        
        migrados = 0
        falhas = 0
        
        # Migrar cada nota
        for matricula, (np1, np2, pim, media) in notas_dict.items():
            try:
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    s.connect((HOST, PORT))
                    request = f"UPDATE_NOTAS|{matricula}|{np1_float}|{np2_float}|{pim_float}|{media_float}"
                    s.sendall(request.encode('utf-8'))
                    resp = s.recv(1024).decode('utf-8')
                    
                    if "SUCESSO" in resp:
                        print(f"   ✓ Migrado: Matrícula {matricula}")
                        migrados += 1
                    else:
                        print(f"   ✗ Falha: Matrícula {matricula} - {resp}")
                        falhas += 1
                        
            except Exception as e:
                print(f"   ✗ Erro ao migrar matrícula {matricula}: {e}")
                falhas += 1
        ...
        # Fazer backup do notas.dat original
        if migrados > 0:
            backup_path = notas_path + f".backup_{int(time.time())}"
            try:
                import shutil
                shutil.copy2(notas_path, backup_path)
                print(f"\n   ✓ Backup criado: {backup_path}")
                
                # Perguntar se deseja deletar o original
                print("\n   ⚠ ATENÇÃO: As notas agora estão no banco C.")
                print(f"   O arquivo {notas_path} pode ser deletado.")
                print("   Um backup foi criado em caso de necessidade.")
...
def verify_sync_consistency():
    """
    Verifica consistência entre banco C e notas.dat
    Útil para debug
    """
    print("\n" + "="*60)
    ...
    # Verificar se notas.dat existe
    if not os.path.exists(notas_path):
        print("   ✓ notas.dat não existe (esperado após migração)")
        print("   ✓ Sistema usando APENAS banco C")
        return True
    
    # Se existir, verificar se está vazio
    notas_dict = load_notas_dat(notas_path)
    
    if not notas_dict:
        print("   ✓ notas.dat vazio")
        print("   ✓ Sistema usando APENAS banco C")
        return True
    
    print(f"   ⚠ ATENÇÃO: notas.dat contém {len(notas_dict)} registros")
    print("   ⚠ Isso pode causar inconsistências!")
    
    # Buscar algumas turmas do servidor
    print("\n   Verificando consistência com servidor...")
    
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            s.sendall(b"LIST_TURMAS")
            resp = s.recv(4096).decode('utf-8')
            
            if "Nenhuma turma" in resp or not resp:
                print("   ⚠ Nenhuma turma encontrada no servidor")
                return False
            
            # Pegar primeira turma
            primeira_linha = resp.strip().split('\n')[0]
            turma_id = primeira_linha.split(', ')[0].split(': ')[1]
            
            print(f"   Verificando turma {turma_id}...")
            
            # Listar alunos da turma
            s2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s2.connect((HOST, PORT))
            s2.sendall(f"LIST_ALUNOS_POR_TURMA|{turma_id}".encode('utf-8'))
            alunos_resp = s2.recv(8192).decode('utf-8')
            s2.close()
            
            if "Nenhum aluno" in alunos_resp:
                print("   ⚠ Nenhum aluno na turma")
                return True
            
            inconsistencias = 0
            
            for linha in alunos_resp.strip().split('\n'):
                if not linha:
                    continue
                ...
                # Extrair notas do servidor
                np1_server = np2_server = pim_server = media_server = 0.0
                for p in partes[2:]:
                    if p.startswith('NP1:'):
                        np1_server = float(p.split(': ')[1])
                    elif p.startswith('NP2:'):
                        np2_server = float(p.split(': ')[1])
                    elif p.startswith('PIM:'):
                        pim_server = float(p.split(': ')[1])
                    elif p.startswith('Média:'):
                        media_server = float(p.split(': ')[1])
                
                # Verificar se existe em notas.dat
                if matricula in notas_dict:
                    np1_dat, np2_dat, pim_dat, media_dat = notas_dict[matricula]
                    
                    # Comparar valores
                    if (float(np1_dat) != np1_server or 
                        float(np2_dat) != np2_server or 
                        float(pim_dat) != pim_server or 
                        float(media_dat) != media_server):
                        
                        print(f"\n   ✗ INCONSISTÊNCIA: Matrícula {matricula}")
                        print(f"      Servidor C: NP1={np1_server}, NP2={np2_server}, PIM={pim_server}, Média={media_server}")
                        print(f"      notas.dat:  NP1={np1_dat}, NP2={np2_dat}, PIM={pim_dat}, Média={media_dat}")
                        inconsistencias += 1
            
            if inconsistencias > 0:
                print(f"\n   ✗ Encontradas {inconsistencias} inconsistências!")
                print("   ⚠ Execute migrate_notas_to_server() para corrigir")
                return False
            else:
...
def cleanup_notas_dat():
    """
    Remove notas.dat após confirmar que o banco C está atualizado
    
    CUIDADO: Execute apenas após migrar e verificar consistência!
    """
    print("\n" + "="*60)
    print("LIMPEZA: Remover notas.dat")
    print("="*60)
    
    notas_path = os.path.join('uploads', 'notas.dat')
    
    if not os.path.exists(notas_path):
        print("   ✓ notas.dat já não existe")
        return
    
    print("\n   ⚠ ATENÇÃO: Esta ação é IRREVERSÍVEL!")
    print("   ⚠ Certifique-se de que executou migrate_notas_to_server()")
    print("   ⚠ E verificou com verify_sync_consistency()")
    
    resposta = input("\n   Deseja REALMENTE deletar notas.dat? (digite 'SIM' para confirmar): ")
    ...
    try:
        ...
        backup_path = notas_path + f".final_backup_{int(time.time())}"
        import shutil
        shutil.copy2(notas_path, backup_path)
        print(f"\n   ✓ Backup final criado: {backup_path}")
        
        # Deletar arquivo
        os.remove(notas_path)
        print(f"   ✓ Arquivo {notas_path} deletado com sucesso")
        print("\n   ✓ Sistema agora usa EXCLUSIVAMENTE o banco C")
...
def export_notas_to_csv():
    """
    Exporta todas as notas do banco C para CSV (backup)
    """
    print("\n" + "="*60)
    print("EXPORTAÇÃO: Notas para CSV")
    print("="*60)
    
    try:
        import csv
        
        output_path = os.path.join('uploads', f'notas_backup_{int(time.time())}.csv')
        
        # Buscar todas as turmas
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            s.sendall(b"LIST_TURMAS")
            turmas_resp = s.recv(4096).decode('utf-8')
        
        if "Nenhuma turma" in turmas_resp:
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(['Turma_ID', 'Disciplina', 'Matrícula', 'Nome', 'NP1', 'NP2', 'PIM', 'Média', 'Exame'])
            ...
            for linha in turmas_resp.strip().split('\n'):
                ...
                partes = linha.split(', ')
                turma_id = partes[0].split(': ')[1]
                disciplina = partes[1].split(': ')[1]
                
                # Buscar alunos da turma
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.connect((HOST, PORT))
                    s.sendall(f"LIST_ALUNOS_POR_TURMA|{turma_id}".encode('utf-8'))
                    alunos_resp = s.recv(8192).decode('utf-8')
                ...
                for aluno_linha in alunos_resp.strip().split('\n'):
                    ...
                    aluno_partes = aluno_linha.split(', ')
                    matricula = aluno_partes[0].split(': ')[1]
                    nome = aluno_partes[1].split(': ')[1]
                    
                    np1 = np2 = pim = media = 0.0
                    for p in aluno_partes[2:]:
                        if p.startswith('NP1:'):
                            np1 = float(p.split(': ')[1])
                        elif p.startswith('NP2:'):
                            np2 = float(p.split(': ')[1])
                    ...
                    # Buscar exame do JSON
                    exame = get_nota_exame(matricula)
                    
                    writer.writerow([turma_id, disciplina, matricula, nome, np1, np2, pim, media, exame])
                    total_alunos += 1
        
        print(f"\n   ✓ Exportados dados de {total_alunos} alunos")
    ...
    def diagnosticar_sistema(self):
        """Função de diagnóstico para verificar o estado do sistema"""
        if self.role != 'admin':
            messagebox.showerror("Acesso Negado", "Apenas administradores podem executar diagnósticos.")
            return
        
        dialog = tk.Toplevel(self)
        dialog.title("Diagnóstico do Sistema")
        dialog.geometry("800x600")
        dialog.transient(self)
        dialog.grab_set()
        _center_window(dialog)
        ...
        tk.Label(main_frame, text="Diagnóstico do Sistema", 
                font=('Arial', 14, 'bold'), bg=self.colors['card'], fg=self.colors['primary']).pack(pady=(0, 20))
        ...
        def executar_diagnostico():
            ...
            text_widget.insert(tk.END, "=== DIAGNÓSTICO DO SISTEMA ===\n\n")
            
            # 1. Verificar usuários
            text_widget.insert(tk.END, "1. USUÁRIOS CADASTRADOS:\n")
            total_usuarios = len(self.db.users)
            admins = sum(1 for u in self.db.users.values() if u.get('role') == 'admin')
            professores = sum(1 for u in self.db.users.values() if u.get('role') == 'professor')
            alunos = sum(1 for u in self.db.users.values() if u.get('role') == 'aluno')
            
            text_widget.insert(tk.END, f"   Total: {total_usuarios}\n")
            text_widget.insert(tk.END, f"   Admins: {admins}\n")
            text_widget.insert(tk.END, f"   Professores: {professores}\n")
            text_widget.insert(tk.END, f"   Alunos: {alunos}\n\n")
            
            # 2. Verificar turmas
            text_widget.insert(tk.END, "2. TURMAS CADASTRADAS:\n")
            turmas = self._get_turmas_list(filtrar_turno=False)
            text_widget.insert(tk.END, f"   Total: {len(turmas)}\n")
            for t in turmas:
                text_widget.insert(tk.END, f"   ID {t['id']}: {t['nome_disciplina']} - {t['nome_professor']}\n")
            text_widget.insert(tk.END, "\n")
            
            # 3. Verificar alunos por turma
            text_widget.insert(tk.END, "3. ALUNOS POR TURMA:\n")
            for t in turmas:
                alunos_turma = self._get_alunos_list(t['id'])
                text_widget.insert(tk.END, f"   Turma {t['id']} ({t['nome_disciplina']}): {len(alunos_turma)} alunos\n")
                for mat, nome in alunos_turma:
                    text_widget.insert(tk.END, f"     - {mat}: {nome}\n")
            text_widget.insert(tk.END, "\n")
            
            # 4. Verificar acesso de professores
            text_widget.insert(tk.END, "4. ACESSO DE PROFESSORES:\n")
            for username, data in self.db.users.items():
                if data.get('role') == 'professor':
                    subjects = data.get('subjects', [])
                    text_widget.insert(tk.END, f"   {username}: {subjects}\n")
            text_widget.insert(tk.END, "\n")
            
            # 5. Verificar alunos sem turma
            text_widget.insert(tk.END, "5. ALUNOS SEM TURMA:\n")
            alunos_sem_turma = 0
            for username, data in self.db.users.items():
                if data.get('role') == 'aluno':
                    matricula = data.get('matricula')
                    if matricula:
                        ...
                        if not resp or "ERRO" in resp:
                            alunos_sem_turma += 1
                            text_widget.insert(tk.END, f"   {username} (matrícula: {matricula})\n")
            text_widget.insert(tk.END, f"   Total: {alunos_sem_turma}\n")
        ...
        btn_executar = tk.Button(button_frame, text="Executar Diagnóstico", 
                               command=executar_diagnostico,
                               bg=self.colors['primary'], fg='white',
                               font=('Arial', 10, 'bold'), relief='flat', cursor='hand2',
                               padx=20, pady=8)
        btn_executar.pack(side=tk.LEFT, padx=(0, 10))
        ...
        dialog.after(100, executar_diagnostico)
...
if __name__ == "__main__":
    ...
    logger = setup_logging()
    logger.info("="*60)
    logger.info("INICIANDO SISTEMA ACADÊMICO")
    logger.info("="*60)
    
    # Inicializar multiprocessing
    if os.name != 'posix':
        multiprocessing.freeze_support()
        multiprocessing.set_start_method('spawn', True)
    
    # Iniciar servidor
    logger.info("Iniciando servidor em processo separado...")
    server_p = multiprocessing.Process(target=run_server, daemon=True)
    server_p.start()
    time.sleep(2)  # Aumentado para 2 segundos (mais seguro)
    
    if not server_p.is_alive():
        logger.error("FALHA CRÍTICA: Servidor não iniciou!")
        logger.error("Verifique se a porta 65432 está disponível")
        logger.error("Ou se database.dll/libdatabase.so está presente")
        print("\n[ERRO] Servidor falhou ao iniciar. Verifique logs/sistema.log")
        exit(1)
    else:
        logger.info("✓ Servidor iniciado com sucesso")
    
    # Verificar consistência na inicialização (apenas aviso, não bloqueia)
    try:
        logger.info("Verificando consistência dos bancos de dados...")
        verify_sync_consistency()
    except Exception as e:
        logger.warning(f"Não foi possível verificar consistência: {e}")
    ...
    try:
        ...
        logger.error(f"Erro na GUI: {e}", exc_info=True)
    finally:
        # Encerrar servidor
        if server_p.is_alive():
            ...
            if server_p.is_alive():
                logger.warning("Servidor não respondeu ao terminate, forçando kill")
                server_p.kill()
