(function () {
  ...
  const els = {
    ...
    chartPie: document.getElementById("chart-pie"),
  ...
  const CHART_COLORS = ["#3d8f7c", "#c45c4c", "#d4a054", "#6b8cae", "#8a6bb5", "#c47a5a", "#5a9b8a"];
  ...
  function renderAvatar() {
    ...
    if (currentAvatar) {
      els.avatarImg.src = currentAvatar;
      ...
      els.avatarInitials.textContent = initialsFrom(currentName);
    }
    if (els.photoHint) {
      els.photoHint.textContent = currentAvatar
        ? "Toque para alterar a foto (galeria ou câmera)"
        : "Toque aqui para enviar uma imagem do celular ou do computador";
    }
    if (els.photoTitle) {
      els.photoTitle.textContent = currentName ? `Foto de ${currentName}` : "Sua foto de perfil";
    }
  }
  ...
  function avatarStorageKey() {
    return "finora-avatar-" + (currentUser && currentUser.id ? currentUser.id : "demo");
  }
  ...
  async function saveAvatar(dataUrl) {
    ...
    if (supabaseClient && currentUser) {
      try {
        await supabaseClient.from("profiles").update({ avatar_url: dataUrl }).eq("id", currentUser.id);
      } catch (err) {
        /* coluna avatar_url pode ainda não existir */
      }
    }
    renderAvatar();
  }
  ...
  async function enterDemo(name) {
    ...
    currentAvatar = state.profile.avatar_url || localStorage.getItem(avatarStorageKey()) || "";
    showView("app");
    await fetchMonth();
  }

  async function loadProfile(user) {
    currentAvatar = localStorage.getItem("finora-avatar-" + user.id) || "";
    if (demoMode) {
      const state = demoState();
      currentAvatar = state.profile.avatar_url || currentAvatar;
      return state.profile.name;
    }

    let data = null;
    let error = null;
    const withAvatar = await supabaseClient
      .from("profiles")
      .select("name, avatar_url")
      .eq("id", user.id)
      .maybeSingle();
    data = withAvatar.data;
    error = withAvatar.error;
    ...
    if (data?.avatar_url) currentAvatar = data.avatar_url;
  ...
  function renderApp() {
    ...
    renderCharts(income, expense, expenseItems);
    } catch (err) {
  ...
  function donutSvg(segments) {
    const svg = svgEl("svg", { viewBox: "0 0 120 120", width: "140", height: "140" });
    const total = segments.reduce((s, part) => s + part.value, 0);
    svg.appendChild(svgEl("circle", {
      cx: "60", cy: "60", r: "42", fill: "none", stroke: "#ece7df", "stroke-width": "16"
    }));
  ...
  function renderCharts(income, expense, expenseItems) {
    if (!els.chartFlow || !els.chartPie) return;
    ...
    [
      { label: "Entradas", value: income, cls: "is-income" },
      { label: "Saídas", value: expense, cls: "is-expense" }
    ].forEach((item) => {
      const col = document.createElement("div");
      col.className = "chart-col";
      const bar = document.createElement("div");
      bar.className = "chart-col-bar " + item.cls;
      bar.style.height = `${Math.max(8, (item.value / max) * 150)}px`;
      const val = document.createElement("strong");
      val.textContent = formatBRL(item.value);
    ...
    els.chartFlow.appendChild(bars);
    ...
    if (!parts.length) {
      const empty = document.createElement("div");
      empty.className = "empty";
      empty.textContent = "As saídas do mês aparecem neste gráfico.";
      els.chartPie.appendChild(empty);
      return;
    }

    const wrap = document.createElement("div");
    wrap.className = "chart-wrap";
    wrap.appendChild(donutSvg(parts));
    ...
    parts.forEach((part) => {
      ...
      const val = document.createElement("span");
      val.textContent = formatBRL(part.value);
      row.append(left, val);
      legend.appendChild(row);
    });
    wrap.appendChild(legend);
    els.chartPie.appendChild(wrap);
  }
  ...
  function bindEvents() {
    ...
    document.getElementById("avatar-input").addEventListener("change", async (e) => {
      ...
      try {
        ...
        toast(err.message || "Não foi possível usar esta imagem.");
      }
    });
